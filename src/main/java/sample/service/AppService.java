...
public class AppService {
    ...
    public List<Algorithm> define(){
        return executor.getSelfDesignNameList().stream().map(
            n->{
    ...
    public List<StockInfo> executeRule(String rule) throws Exception {
        ...
        List<StockInfo> stockInfoList = DatabaseManager.getInstance().selectByIds(stockIdList);
        stockInfoList.stream().forEach(s ->{
            List<DailyIndex> dailyIndices = map.get(s.getId());
            DailyIndex last = dailyIndices.get(dailyIndices.size() - 1);
            s.setPrice(last.getClosingPrice());
            s.setRiseInfo(StockUtil.getRise(last)+"");
        });
    ...
    public Map<Integer, List<DailyIndex>> prepareMarketData() throws Exception {
