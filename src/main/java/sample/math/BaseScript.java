...
public abstract class BaseScript {
    ...
    public String getName(){
        if (name == null){
            StockInfo ss = null;
            try {
                ss = DatabaseManager.getInstance().selectByIds(Arrays.asList(dailyIndices.get(0).getStockInfoId())).get(0);
            } catch (Exception e) {
                e.printStackTrace();
            }
            name = ss.getName();
    ...
    public final boolean ZT(int offset){
        int realOffset = CURRENT_DATE + offset;
        DailyIndex dailyIndex = dailyIndices.get(realOffset);
        return StockUtil.isZhangting(dailyIndex);
    }

    public final boolean DT(int offset){
        int realOffset = CURRENT_DATE + offset;
        DailyIndex dailyIndex = dailyIndices.get(realOffset);
        return StockUtil.isDiegting(dailyIndex);
    }
