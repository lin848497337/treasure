...
import static  sample.util.StockConsts.StockState;
...
public class StockUtil {
    ...
    public static boolean isZhangting(DailyIndex dailyIndex){
        return dailyIndex.getPreClosingPrice().multiply(new BigDecimal("1.1")).setScale(2, RoundingMode.HALF_UP).equals(dailyIndex.getClosingPrice());
    }

    public static boolean isGreenKey(DailyIndex dailyIndex){
        return dailyIndex.getOpeningPrice().compareTo(dailyIndex.getClosingPrice()) > 0;
    }
    ...
    public static double getRise(DailyIndex dailyIndex){
        return dailyIndex.getClosingPrice().subtract(dailyIndex.getPreClosingPrice()).divide(dailyIndex.getPreClosingPrice(), 2, RoundingMode.HALF_UP).doubleValue();
    }

    public static double shakeClosePrice(DailyIndex dailyIndex){
        return dailyIndex.getHighestPrice().subtract(dailyIndex.getClosingPrice()).divide(dailyIndex.getPreClosingPrice(), 2, RoundingMode.HALF_UP).doubleValue();
    }
